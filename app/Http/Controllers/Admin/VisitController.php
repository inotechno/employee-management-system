...
class VisitController extends Controller
{
    ...
    {
        return view('visits.employee');
    }

    public function site()
    {
        $sites = Site::all();
        return view('visits.site', compact('sites'));
    }
    ...
    public function datatable_sites(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($visits)
                ...
                ->addColumn('action', function ($row) {
                    return '<button class="btn btn-info btn-sm btn-detail" data-site_id="' . $row->site_id . '" data-name="' . $row->site->name . '" data-month="' . $row->month . '" data-year="' . $row->year . '">
                               <i class="bx bx-detail"></i> Detail
                            </button>';
                })
                ->addIndexColumn()
    ...
    //         $sites = Visit::with('site')->where(['employee_id' => $request->employee_id, 'status' => 0])->whereMonth('created_at', $request->month)->whereYear('created_at', $request->year)->get();
    //     }

    //     if ($request->site_id) {
    //         $sites = Visit::with('employee', 'employee.user')->where(['site_id' => $request->site_id, 'status' => 0])->whereMonth('created_at', $request->month)->whereYear('created_at', $request->year)->get();
    //     }

    //     $sites->date = $sites->map(function ($row) {
    //         $row->date = Carbon::parse(date('Y-m-d H:i', strtotime($row->created_at)))->isoFormat('LLLL');
    //         return $row;
    //     });
